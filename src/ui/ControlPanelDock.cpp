...
void ControlPanelDock::resizeEvent(QResizeEvent *event) {
    ...
    control_panel->border = control_panel->border_base.scaled(event->size().width(), event->size().height() * 0.8, Qt::AspectRatioMode::KeepAspectRatio, Qt::TransformationMode::FastTransformation);
