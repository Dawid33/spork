...
void StatusConsoleDock::resizeEvent(QResizeEvent *event) {
    ...
    console->border = console->border_base.scaled(event->size().width(), event->size().height() * 6, Qt::AspectRatioMode::KeepAspectRatio, Qt::TransformationMode::FastTransformation);
    console->setFixedHeight(console->border.height());
}